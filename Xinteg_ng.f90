...
! ------------------------------
  function IFCT_NG_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha,alpha_s,nlo,LUMI,lumi_ex
    ...
    if ( (abs(m1)+abs(mg))**2 > 0.98d0*sc ) then
!tp       print*, " collider energy not large enough ",m1,mg,sqrt(sc)
    ...
    if (isca==0) then                                          ! renormalization/factorization scale
       qf = scafac * (m1+mg)/2.0d0
       qr = scafac * (m1+mg)/2.0d0
    else if (isca==1) then
       qf = scafac * sqrt(s)
       qr = scafac * sqrt(s)
    end if
    ...
    if ((s>(ms+mg)**2).and.(ms>m1)) theta_s4 = 1.0d0 
    if ((s>(ms+m1)**2).and.(ms>mg)) theta_s3 = 1.0d0 

    del_s4 = eps_sli * min(m1**2,mg**2)                       &! unit is m^2
                   * (1.0d0-(m1+mg)**2/s)*(1.0d0-(m1-mg)**2/s)     ! rescale the s4 cutoff to be small everywhere 
    s4p    = 0.0d0                                               ! only for real corrections needed 

    select case (ii)                                           ! all the phase spaces 
    case(-1,0,1,2)                                             ! born_lo, born_nlo, virt

       beta   = sqrt(1.0d0-(m1+mg)**2/s) * sqrt(1.0d0-(m1-mg)**2/s)               ! tg integration
       tgm    = -1.0d0/2.0d0 * ( s + mg**2 - m1**2 + s*beta )                
       tgp    = -1.0d0/2.0d0 * ( s + mg**2 - m1**2 - s*beta )
       tg     = var(3) * (tgp-tgm) + tgm
       tg_jac = tgp-tgm 

    case(3)                                                                    ! real(qb), no os subtraction 
       
       beta   = sqrt((1.0d0-del_s4/s)**2-(m1+mg)**2/s)                   &       ! tg integration shifted 
               *sqrt((1.0d0-del_s4/s)**2-(m1-mg)**2/s)                    
       tgm    = -1.0d0/2.0d0 * ( s - del_s4 + mg**2 - m1**2 + s*beta )
       tgp    = -1.0d0/2.0d0 * ( s - del_s4 + mg**2 - m1**2 - s*beta )
       tg     = var(3) * (tgp-tgm) + tgm
       tg_jac = tgp-tgm 
    ...
    case(4)                                                                    ! real(qg+gb) w/o os subtraction
       ...
       s4m    = 0.0d0                                                            ! s4 mapped to atan
       s4p    = s + mg**2 - m1**2 - 2.0d0 * sqrt(s*mg**2)
       ...
       beta   = sqrt( (s-s4-m1**2+mg**2)**2 - 4.0d0*s*mg**2 )
       tgm    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + mg**2 + beta )                   ! tg integration as usual
       tgp    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + mg**2 - beta )
    ...
    case(5)                                                                    ! real(qg+gb) os subtraction
       ...
       if (theta_s4==1.0d0) then                                                 ! restricted phase space for theta_s4=1 
          ...
          betas4   = sqrt( (s-s4s4-m1**2+mg**2)**2 - 4.0d0*s*mg**2 )
          tgs4m    = -1.0d0/2.0d0 * ( s - s4s4 - m1**2 + mg**2 + betas4 )
          tgs4p    = -1.0d0/2.0d0 * ( s - s4s4 - m1**2 + mg**2 - betas4 )
       ...
       s3p    = s + m1**2 - mg**2 - 2.0d0 * sqrt(s*m1**2) 
       ...
       beta1   = sqrt( (s-s3-mg**2+m1**2)**2 - 4.0d0*m1**2*s )
       s4xm    = s3/2.0d0/(s3+mg**2) * (s - s3 - m1**2 - mg**2 - beta1)
       s4xp    = s3/2.0d0/(s3+mg**2) * (s - s3 - m1**2 - mg**2 + beta1)
       s4x     = var(4) * (s4xp-s4xm) + s4xm
       s4x_jac = s4xp-s4xm
       
       beta2  = sqrt( (s-s4x-m1**2+mg**2)**2 - 4.0d0*mg**2*s ) 
       tgxm    = -1.0d0/2.0d0 * ( s - s4x - m1**2 + mg**2 + beta2 )                ! angular integration re-written
       tgxp    = -1.0d0/2.0d0 * ( s - s4x - m1**2 + mg**2 - beta2 )
       tgx     = var(5) * (tgxp-tgxm) + tgxm 
       tgx_jac = tgxp-tgxm 

       tgx_jac = tgx_jac * 2.0d0*(s4x+m1**2)/s4x/beta2                           ! the additional jacobian

       if (theta_s3==1.0d0) then                                                 ! the complete s3 type phase space
          s3s3 = ms**2 - mg**2                                                 ! restricted phase space for theta_s3=1 
         
          beta1s3   = sqrt( (s-s3s3-mg**2+m1**2)**2 - 4.0d0*m1**2*s )
          s4s3m    = s3s3/2.0d0/(s3s3+mg**2) * (s - s3s3 - m1**2 - mg**2 - beta1s3)
          s4s3p    = s3s3/2.0d0/(s3s3+mg**2) * (s - s3s3 - m1**2 - mg**2 + beta1s3)
          s4s3     = var(4) * (s4s3p-s4s3m) + s4s3m
          s4s3_jac = s4s3p-s4s3m

          beta2s3  = sqrt( (s-s4s3-m1**2+mg**2)**2 - 4.0d0*s*mg**2 ) 
          tgs3m    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 + mg**2 + beta2s3 )         ! angular integration re-written
          tgs3p    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 + mg**2 - beta2s3 )
          tgs3     = var(5) * (tgs3p-tgs3m) + tgs3m 
          tgs3_jac = tgs3p-tgs3m 

          tgs3_jac = tgs3_jac * 2.0d0*(s4s3+m1**2)/s4s3/beta2s3
    ...
    massin(26) = 1.0d0                                           ! to tag 1/s4^2 term only for ii=8,10 
    massin(27) = 0.0d0                                           ! to tag 1/s3^2 term only for ii=8,10
    ...
    if (ii==5) then                                            ! define restricted phase space 
       ...
       massix(3)       = s4x
       massix(4)       = s3
       massix(26)      = 0.0d0                                   ! to tag 1/s4^2 term 
       massix(27)      = 1.0d0                                   ! to tag 1/s3^2 term 
    ...
    if (iscaling==0) dsig = dsig * 4.0d0/(m1+mg)**2 * 4.D0*pi*alpha * 4.D0*pi*alpha_s
...
! ------------------------------
  subroutine COUPLING_NG(iq,Cl,Cr,Cv)
    ...
    Cl(1:4) = 0.0d0                                                            ! initialize arrays
    Cr(1:4) = 0.0d0
    Cv(1:4) = 0.0d0
    ...
    case(1,2,3,4)                                                            ! neutralino and gluino 
       ...
       Cl(1) = ( zz(ipart1,1)*sw*(qq(iq)-t3(iq))+zz(ipart1,2)*cw*t3(iq))/(sqrt(2.0d0)*sw*cw) ! nqs coupings [gauginos]
       Cr(1) =  zzc(ipart1,1)*sw* qq(iq)                                /(sqrt(2.0d0)*sw*cw) ! in wim's normalization 
       Cl(3) = Cl(1)
       Cr(3) = Cr(1) 

    case(5,6)                                                                ! chargino+ and gluino

       Cl(1) = uu(ic,1)/(2.0d0*sw)                                             ! t channel with s-down for C+
       Cl(3) = vv(ic,1)/(2.0d0*sw)                                             ! u channel with s-up for C+

    case(7,8)                                                                ! chargino- and gluino

       Cl(3) = uu(ic,1)/(2.0d0*sw)                                             ! t channel with s-up for C-
       Cl(1) = vv(ic,1)/(2.0d0*sw)                                             ! u channel with s-down for C-
