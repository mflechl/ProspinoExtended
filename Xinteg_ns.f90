...
! ------------------------------
  function IFCT_NS_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha,alpha_s,nlo
    ...
    charge(-6:6) = 99.0d0                                        ! charge of the quark flavors (according to cteq)
    charge( 1)   =  2.0d0/3.0d0                                    ! u quark
    charge(-1)   = -2.0d0/3.0d0                                    ! u antiquark
    charge( 2)   = -1.0d0/3.0d0                                    ! d quark
    charge(-2)   =  1.0d0/3.0d0                                    ! d antiquark
    charge( 3)   = -1.0d0/3.0d0                                    ! s quark
    charge(-3)   =  1.0d0/3.0d0                                    ! s antiquark
    charge( 4)   =  2.0d0/3.0d0                                    ! c quark
    charge(-4)   = -2.0d0/3.0d0                                    ! c antiquark
    charge( 5)   = -1.0d0/3.0d0                                    ! b quark
    charge(-5)   =  1.0d0/3.0d0                                    ! b antiquark
...
!tp       print*, " collider energy not large enough ",m1,m2,sqrt(sc)
    ...
    if (isca==0) then                                          ! renormalization/factorization scale
       qf = scafac * (abs(m1)+abs(m2))/2.0d0
       qr = scafac * (abs(m1)+abs(m2))/2.0d0
    else if (isca==1) then
       qf = scafac * sqrt(s)
       qr = scafac * sqrt(s)
    end if
    ...
    if ((s>(mg+abs(m2))**2).and.(mg>    m1 )) theta_s4 = 1.0d0   ! intermediate gluino (gluino-neutralino associated) 
    if ((s>(ms+    m1 )**2).and.(ms>abs(m2))) theta_s3 = 1.0d0   ! intermediate squark (squark pair)

    del_s4 = eps_sli * min(m1**2,m2**2)                                    &    ! unit is m^2
                   * (1.0d0-(abs(m1)+abs(m2))**2/s)*(1.0d0-(abs(m1)-abs(m2))**2/s)  ! rescale the s4 cutoff 
    if (ii/=3) del_s4 = 0.0d0

    s4p    = 0.0d0                                                               ! only for real corrections needed 

    select case (ii)                                                           ! all the phase spaces 
    case(-1,0,1,2)                                                             ! born_lo, born_nlo, virt

       beta   = sqrt(1.0d0-(abs(m1)+abs(m2))**2/s)                            &  ! t2 integration
               *sqrt(1.0d0-(abs(m1)-abs(m2))**2/s)      
       t2m    = -1.0d0/2.0d0 * ( s + m2**2 - m1**2 + s*beta )                
       t2p    = -1.0d0/2.0d0 * ( s + m2**2 - m1**2 - s*beta )
       t2     = var(3) * (t2p-t2m) + t2m
       t2_jac = t2p-t2m 

    case(3,4)                                                                ! real(qg), crossed(qbq), crossed(qb), no os subtraction 
       
       beta   = ((1.0d0-del_s4/s)**2-(abs(m1)+abs(m2))**2/s)                  &  ! t2 integration shifted 
               *((1.0d0-del_s4/s)**2-(abs(m1)-abs(m2))**2/s)                    
       if (beta>=0.0d0) then 
          beta = sqrt(beta)
       else 
       ...
       t2m    = -1.0d0/2.0d0 * ( s - del_s4 + m2**2 - m1**2 + s*beta )
       t2p    = -1.0d0/2.0d0 * ( s - del_s4 + m2**2 - m1**2 - s*beta )
       t2     = var(3) * (t2p-t2m) + t2m
       t2_jac = t2p-t2m 
...
!       s4_jac = s4 * log(s4p/s4m)
    ...
    case(6)                                                                    ! real(qb), real(qq) with os s4g subtraction
       ...
       beta   = sqrt( dabs( (s-s4-m1**2+m2**2)**2 - 4.0d0*s*m2**2 ) )            ! absolute value because of numerics in subtraction
       t2m    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + m2**2 + beta )                   ! t2 integration as usual
    ...
    case(5,7)                                                                  ! real(gg), crossed(qq), crossed(qb) os subtraction s3s 
       ...
       s4xm    = s3/2.0d0/(s3+m2**2) * (s - s3 - m1**2 - m2**2 - beta1)
       s4xp    = s3/2.0d0/(s3+m2**2) * (s - s3 - m1**2 - m2**2 + beta1)
       s4x     = var(4) * (s4xp-s4xm) + s4xm
       s4x_jac = s4xp-s4xm

       beta2   = sqrt( (s-s4x-m1**2+m2**2)**2 - 4.0d0*m2**2*s ) 
       t2xm    = -1.0d0/2.0d0 * ( s - s4x - m1**2 + m2**2 + beta2 )                ! angular integration re-written
       t2xp    = -1.0d0/2.0d0 * ( s - s4x - m1**2 + m2**2 - beta2 )
       t2x     = var(5) * (t2xp-t2xm) + t2xm 
       t2x_jac = t2xp-t2xm 

       t2x_jac = t2x_jac * 2.0d0*(s4x+m1**2)/s4x/beta2                           ! the additional jacobian
       ...
       if (theta_s3==1.D0) then 
          ...
          t2s3_jac = t2s3_jac * 2.0d0*(s4s3+m1**2)/s4s3/beta2s3
    ...
    if (ii<=5) then                                                                   ! as simple only for simple lumis
       ...
       do ih  = -1,+1,2                                                                ! loop for the squark helicities (-1=left, +1=right)
          ...
          case(-1)                                                                    ! born
             ...
             if ( (C1(1)*C1(2))/=0.0d0 )                                               &! make sure this is not zero anyway
             dsig = dsig + lumi_ex(1) /s**2 * NS_QGB(massin,C1) * t2_jac              ! quark-gluon

             if ( (C2(1)*C2(2))/=0.0d0 )                                               &! antiquark-gluon
       ...
       do isq = 1,4,1                                               ! always produce squark, not anti-squark
          ...
          if (iq1==iq2) symmfac = 0.5d0                               ! identical quarks in the qq initial state
          ...
          if ( abs(sum_charge)>0.01d0 ) cycle 
    ...
    if (iscaling==0)                                                                   &  ! scaling function flag
         dsig = dsig * 4.0d0/(abs(m1)+abs(m2))**2 * 4.D0*pi*alpha * 4.D0*pi*alpha_s

    if (iscaling==0)                                                                   &  ! result in pb 
         dsig = dsig * gevpb

    if ( (ii==-1).and.(dsig==0.0d0) ) ii_done(-2) = 0                                       ! only needed for this channel
...
!  Cx includes Cj,Cjp,Cjf,Cjpf
  subroutine COUPLING_NS_EXT(ih,Cx)
    ...
    t3(1:5) = (/ +1.0d0,-1.0d0,-1.0d0,+1.0d0,-1.0d0 /)                                 ! quarks: u,d,s,c,b

    qq(1:5) = (/ +2.0d0,-1.0d0,-1.0d0,+2.0d0,-1.0d0 /)
    qq(1:5) = qq(1:5)/3.0d0
    ...
    Cx(1:5,1:5,1:4) = 0.0d0                                                    ! initialize arrays
    ...
    case(1,2,3,4)                                                            ! neutralino
       ...
       do n1=1,5,1
          do n2=1,5,1
             if (n1/=n2) Cx(n1,n2,1:4) = 0.0d0                                 ! only flavor diagonal couplings 
             if (ih==-1) then 
    ...
    case(5,6)                                                                ! chargino+
       ...
       if (ih==-1) then                                                      ! only left coupling 
          Cx(1:5,1:5,1) = uu(ic,1)/(sqrt(2.0d0)*sw)
          Cx(1:5,1:5,2) = vv(ic,1)/(sqrt(2.0d0)*sw)
       end if

       if (ih==+1) then
          Cx(1:5,1:5,3) = uu(ic,1)/(sqrt(2.0d0)*sw)
          Cx(1:5,1:5,4) = vv(ic,1)/(sqrt(2.0d0)*sw)
       end if

    case(7,8)                                                                ! chargino-

       if (ih==-1) then                                                      ! only left coupling  
          Cx(1:5,1:5,1) = vv(ic,1)/(sqrt(2.0d0)*sw)
          Cx(1:5,1:5,2) = uu(ic,1)/(sqrt(2.0d0)*sw)
       end if

       if (ih==+1) then                                                      ! only left coupling  
          Cx(1:5,1:5,3) = vv(ic,1)/(sqrt(2.0d0)*sw)
          Cx(1:5,1:5,4) = uu(ic,1)/(sqrt(2.0d0)*sw)
       end if
