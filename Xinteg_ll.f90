...
! ------------------------------
  function IFCT_LL_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha,nlo,LUMI
...
!tp       print*, " collider energy not large enough ",m1,m2,sqrt(sc)
    ...
    delta_soft = eps_sub * (m1+m2)**2/4.0d0            ! the soft cut-off rescaled 
    ...
    case(-1,0,1,2)                                             ! t2 integration for born, virtual 
       ...
       beta = sqrt(1.0d0-(m1+m2)**2/s) * sqrt(1.0d0-(m1-m2)**2/s)
...
!tp       mu = max( mu, m1+m2 )
    ...
    case(4,6)                                                               ! t2-x integration for mass factorization
       ...
       betax = sqrt(1.0d0-(m1+m2)**2/sx) * sqrt(1.0d0-(m1-m2)**2/sx)

       t2xm    = -1.0d0/2.0d0 * ( sx + m2**2 - m1**2 + sx*betax )  ! redo t2 integration 
       t2xp    = -1.0d0/2.0d0 * ( sx + m2**2 - m1**2 - sx*betax )
    ...
    if (iscaling==0) dsig = dsig * 4.0d0/(m1+m2)**2 * alpha**2 
...
!
  subroutine KIN_LL(m1,m2,s,t2,s4,c1_in,ax,s3,s5,u2,t1,u1,tp,up)
    ...
    if (test(0)>1.d-6) print *," KIN_LL: test0 better be zero "
    if (test(1)>1.d-6) print *," KIN_LL: test1 better be zero "
    if (test(2)>1.d-6) print *," KIN_LL: test2 better be zero "
    if (test(3)>1.d-6) print *," KIN_LL: test3 better be zero "
    if (test(4)>1.d-6) print *," KIN_LL: test4 better be zero "
    if (test(5)>1.d-6) print *," KIN_LL: test5 better be zero "
...
! ------------------------------
  subroutine COUPLING_LL(s,m1,m2,iq,Cs,Cv,mx,taumix,mkraemer)
    ...
    if ((ipart1<=3).or.((ipart1>=6).and.(ipart1<=9)).or.(ipart1==14) ) then ! neutral final state
       ...
       select case(ipart1)
       case(0)
          v2w  = - ( t3w          - 2.0d0*sw2 * qqw )/(2.0d0*sw*cw) 
          a2w  = -   t3w                           /(2.0d0*sw*cw) 
          gamfac = 2.0d0 
       case(1,3,9)
          v2w  = - ( t3w         -      sw2 * qqw )/(2.0d0*sw*cw) 
          a2w  = v2w
       case(2)
          v2w  = - ( 0.0d0         -      sw2 * qqw )/(2.0d0*sw*cw) 
          a2w  = - v2w
       case(6)                                                               ! stau1-stau1*
          v2w  = - ( t3w*taumix(1,1)**2 - sw2 * qqw )/(2.0d0*sw*cw) 
          a2w  = v2w
       case(7)                                                               ! stau2-stau2*
          v2w  = - ( t3w*taumix(2,1)**2 - sw2 * qqw )/(2.0d0*sw*cw) 
          a2w  = v2w
       case(8)                                                               ! stau1-stau2* plus stau2-stau1*
          v2w = - t3w*taumix(1,1)*taumix(2,1) /(2.0d0*sw*cw)
          a2w  = v2w
       case(14)
          v2w  = -(cw2-sw2)/2.0d0                    /(2.0d0*sw*cw)
          a2w  = 0.0d0
       end select
       ...
       select case(ipart1)
       case(4,5)
          v2w  = - 1.0d0 /(2.0d0*sqrt(2.0d0)*sw)
       case(10,11)                                                               ! sntau-stau1
          v2w  = - taumix(1,1) * 1.0d0 /(2.0d0*sqrt(2.0d0)*sw)
       case(12,13)                                                               ! sntau-stau2
          v2w  = - taumix(2,1) * 1.0d0 /(2.0d0*sqrt(2.0d0)*sw)
       end select
       a2w = v2w
    ...
    Cs(1) =  gamfac * (v1*v1w)**2 /2.0d0 + s * 2.0d0 * real( v1*v1w*v2*v2w )/(s-mx**2)  &  ! wim's conventions 
    ...
    mkraemer(21:28) = 0.0d0

    only_u = ( 1.0d0 + iq )/2.0d0                                                           ! quarks in the t/u channel 
    ...
    Cs(1) = 16.0d0 * pi**2    * Cs(1)                                                     ! rescaling g^2 -> alpha
