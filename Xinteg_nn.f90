...
! ------------------------------
  function IFCT_NN_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha,nlo,LUMI,lumi_ex
...
!    var(1:dim(ii)) = 0.5
    ...
    massin(1:30) = 0.0d0                                         ! initialize the massin array
    ...
    if ( (abs(m1)+abs(m2))**2 > 0.98d0*sc ) then
!tp       print*, " collider energy not large enough ",m1,m2,sqrt(sc)
       dsig = 0.0d0
       return
    end if

    delta_soft = eps_sub * (abs(m1)+abs(m2))**2/4.0d0            ! the soft cut-off rescaled 
    ...
    sw    = sqrt( 1.0d0 - mw**2/mz**2 )                          ! weak parameters in the on-shell scheme 
    alpha = sqrt(2.0d0) * mw**2 * sw**2 /pi * gf  
    ...
    theta_s4 = 0.0d0                                             ! the os subtraction theta function 
    theta_s3 = 0.0d0 
    gams = ewi * ms 
    if ((s>(ms+abs(m2))**2).and.(ms>abs(m1))) theta_s4 = 1.0d0
    if ((s>(ms+abs(m1))**2).and.(ms>abs(m2))) theta_s3 = 1.0d0 
...
!    print*,m1," ",m2," ",ms," ",s
    ...
    case(-1,0,1,2)                                             ! t2 integration for born, virtual 
       ...
       beta = sqrt(1.0d0-(abs(m1)+abs(m2))**2/s) * sqrt(1.0d0-(abs(m1)-abs(m2))**2/s)

       t2m    = -1/2.0d0 * ( s + m2**2 - m1**2 + s*beta )
       t2p    = -1/2.0d0 * ( s + m2**2 - m1**2 - s*beta )
...
!tp       that = t2 + m2**2
!tp       uhat = u2 + m2**2
!tp       mu = sqrt( 2.0d0*shat*that*uhat/(shat**2+that**2+uhat**2) )
!tp       mu = max( mu, m1+m2 )

!      print*, beta, " ", t2m, " ", t2p, " ", t2, " ", t2_jac, " ", u1, " ", t1, " ", u2
    ...
    case(3)                                                    ! t2-s4-omega integration for real(qb)
       ...
       if ((abs(tp)<delta_soft).or.(abs(up)<delta_soft).or.(abs(s3)<delta_soft).or.(abs(s4)<delta_soft)) then 
          dsig = 0.0d0                                           ! cut the soft phase space 
          return 
       end if
    ...
    case(4,5)                                                               ! t2-x integration for mass factorization
       ...
       betax = sqrt(1.0d0-(abs(m1)+abs(m2))**2/sx) * sqrt(1.0d0-(abs(m1)-abs(m2))**2/sx)

       t2xm    = -1/2.0d0 * ( sx + m2**2 - m1**2 + sx*betax )    ! redo t2 integration 
       t2xp    = -1/2.0d0 * ( sx + m2**2 - m1**2 - sx*betax )
    ...
    case(6,7,9,10)                                             ! z-t2-omega for real subtracted (qg,gb)
       ...
       s4m    = 0.0d0                                            ! s4 integration mapped to atan
       s4p    = s + m2**2 - m1**2 - 2.0d0 * sqrt(s*m2**2)
       ...
       beta = sqrt( (s-s4-m1**2+m2**2)**2 - 4.0d0*s*m2**2 )

       t2m    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + m2**2 + beta )
       t2p    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + m2**2 - beta )
       ...
       if (theta_s4==1.0d0) then                                    ! restricted phase space for theta_s4=1

          s4s4 = ms**2 - m1**2

          betas4 = sqrt( (s-s4s4-m1**2+m2**2)**2 - 4.0d0*s*m2**2 )

          t2s4m    = -1.0d0/2.0d0 * ( s - s4s4 - m1**2 + m2**2 + betas4 )
          t2s4p    = -1.0d0/2.0d0 * ( s - s4s4 - m1**2 + m2**2 - betas4 )
    ...
    case(8,11)
       ...
       if (theta_s3==0.0d0) then                                     ! what it's all about 
          dsig = 0.0d0 
          return
       end if

       s3m    = 0.0d0                                                ! re-written integration in general
       s3p    = s + m1**2 - m2**2 - 2.0d0 * sqrt(s*m1**2)
       ...
       beta1   = sqrt( (s-s3-m2**2+m1**2)**2 - 4.0d0*m1**2*s )
       s4m    = s3/2.0d0/(s3+m2**2) * (s - s3 - m1**2 - m2**2 - beta1)
       s4p    = s3/2.0d0/(s3+m2**2) * (s - s3 - m1**2 - m2**2 + beta1)
       s4     = var(4) * (s4p-s4m) + s4m
       s4_jac = s4p-s4m
       
       beta2  = sqrt( (s-s4-m1**2+m2**2)**2 - 4.0d0*s*m2**2 ) 
       t2m    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + m2**2 + beta2 )
       t2p    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + m2**2 - beta2 )
       t2     = var(5) * (t2p-t2m) + t2m 
       t2_jac = t2p-t2m 
       
       t2_jac = t2_jac * 2.0d0/s4*(s4+m1**2) / beta2                          ! including the over-all jacobian
       ...
       beta1s3   = sqrt( (s-s3s3-m2**2+m1**2)**2 - 4.0d0*m1**2*s )
       s4s3m    = s3s3/2.0d0/(s3s3+m2**2) * (s - s3s3 - m1**2 - m2**2 - beta1s3)
       s4s3p    = s3s3/2.0d0/(s3s3+m2**2) * (s - s3s3 - m1**2 - m2**2 + beta1s3)
       s4s3     = var(4) * (s4s3p-s4s3m) + s4s3m
       s4s3_jac = s4s3p-s4s3m
       
       beta2s3  = sqrt( (s-s4s3-m1**2+m2**2)**2 - 4.0d0*s*m2**2 ) 
       t2s3m    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 + m2**2 + beta2s3 )         ! angular integration re-written
       t2s3p    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 + m2**2 - beta2s3 )
       t2s3     = var(5) * (t2s3p-t2s3m) + t2s3m 
       t2s3_jac = t2s3p-t2s3m 
       
       t2s3_jac = t2s3_jac * 2.0d0/s4s3*(s4s3+m1**2) / beta2s3                ! including the over-all jacobian
    ...
    massin(26) = 1.0d0                                           ! yes/no for s4 o-s subtraction
    massin(27) = 1.0d0                                           ! yes/no for s3 o-s subtraction 
    massin(30) = ewi                                           ! additional entry for crossed channel 

!    print*,massin

    mkraemer(1:99) = 0.0d0                                       ! the array mkraemer for real correction 
    mkraemer(1)  = m1 
    mkraemer(2)  = m2 
    mkraemer(3)  = mg 
    mkraemer(4)  = ms 
    ...
    if (ii>=6) then                                            ! define restricted phase space 
       if (theta_s4==1.0d0) then 
          massin_x4(1:30) = massin(1:30) 
          massin_x4(26)   = 1.0d0                                ! only the s4 os subtraction 
          massin_x4(27)   = 0.0d0
       ...
       if (theta_s3==1.0d0) then 
          massin_x3(1:30) = massin(1:30) 
          massin_x3(26)   = 0.0d0                                ! only the s4 os subtraction 
          massin_x3(27)   = 1.0d0
...
!    print*,pdf1,pdf2
    ...
    do iq =-1,1,2                                                         ! the loop for up-type and down-type quarks 
       ...
       if ( (ii>=0) .and. (2.0d0*ms/(abs(m1)+abs(m2))>20.0d0) ) then          ! no t channel couplings
          Cl(1:4) = 0.0d0                           
          Cr(1:4) = 0.0d0 
          Cv(1:4) = 0.0d0
       end if

       if ( (ii>=0) .and. ((2.0d0*ms/(abs(m1)+abs(m2))>100.0d0).and.(ms>1.d4)) ) then ! no subtraction of intermediate squarks
          massin(26) = 0.0d0                        
          massin(27) = 0.0d0
       end if
...
!          if ((theta_s4==0.0d0).and.(theta_s3==0.0d0)) then                                       ! moved to ii=7
!          dsig = dsig + nlo *   LUMI(inlo,30,icoll,idub,iq,x1,x2,mu)                          &
!                            * NN_QGHSUB(massin,mkraemer)  /s**2 * s4 / (s4+m1**2)             &
!                                                        * t2_jac * s4_jac * c1_jac * ax_jac
!          end if 
       case(7)                                                                                 ! s4-matrix qg
          if ((theta_s4==0.0d0).and.(theta_s3==0.0d0)) then
          dsig = dsig + nlo *   LUMI(inlo,30,icoll,idub,iq,x1,x2,mu)                          &
                            * NN_QGHSUB(massin,mkraemer)  /s**2 * s4 / (s4+m1**2)             &
                            * t2_jac * s4_jac * c1_jac * ax_jac
          else if ((theta_s4==1.0d0).and.(theta_s3==0.0d0)) then 
          dsig = dsig + nlo *   LUMI(inlo,30,icoll,idub,iq,x1,x2,mu)                          &
                          * ( NN_QGHSUB(massin,mkraemer)     /s**2 * s4   / (s4+m1**2)        &
                                                        * t2_jac * s4_jac * c1_jac * ax_jac   &
                            - NN_QGHSUB(massin_s4,mkraemer)  /s**2 * s4s4 / (s4s4+m1**2)      &
                                            * prop_s4 * t2s4_jac * s4_jac * c1_jac * ax_jac )             
          else if ((theta_s4==1.0d0).and.(theta_s3==1.0d0)) then 
          dsig = dsig + nlo *   LUMI(inlo,30,icoll,idub,iq,x1,x2,mu)                          &
       ...
       case(8)                                                                                 ! s3-matrix qg
          if ((theta_s4==0.0d0).and.(theta_s3==1.0d0)) then
          dsig = dsig + nlo *   LUMI(inlo,30,icoll,idub,iq,x1,x2,mu)                          &
                          * ( NN_QGHSUB(massin,mkraemer)  /s**2 * s4   / (s4+m1**2)           &
...
!          end if
    ...
    if (iscaling==0) dsig = dsig * 4.0d0/(abs(m1)+abs(m2))**2 * alpha**2 
...
!
  subroutine KIN_NN(type,m1,m2,s,t2,s4,c1_in,ax,s3,s5,u2,t1,u1,tp,up)
    ...
    w2 = ( s + t2 )                       /norm
    w3 =  s4                              /norm
    e1 = ( s4 + 2.0d0*m1**2 )               /norm
    e2 = - ( t2 + u2 + 2.0d0*m2**2 )        /norm
    p  = sqrt( (t2+u2)**2 - 4.0d0*m2**2*s ) /norm
    cx = ( t2 * ( s4+m1**2-m2**2 ) - s*( u2+2.0d0*m2**2 ) )/ ( s+t2 ) / sqrt( (t2+u2)**2 - 4.0d0*m2**2*s )
    ...
    if (type==0) then 
       t1 = 2.0d0 * ( - w1*e1 - w3*s1*c2*p*sx - w3*p*cx*c1 + w3*w2*c1 )
       u1 = 2.0d0 * ( - w2*e1 - w2*w3*c1 ) 
       tp = 2.0d0 * ( - w2*w3 + w2*w3*c1 )
       up = 2.0d0 * ( - w1*w3 + w3*p*sx*s1*c2 + w3*p*cx*c1 - w2*w3*c1 )
       s3 = 2.0d0 * ( e2*w3  - w3*p*sx*s1*c2 - w3*p*cx*c1 )
       s5 = 2.0d0 * ( e1*e2 + w3*p*sx*s1*c2 + w3*p*c1*cx )+m1**2+m2**2 
    else if (type==1) then 
       t1 = 2.0d0 * ( - w1*e1 + w2*w3*s1*c2*sx + w2*w3*cx*c1 - w3*p*c1 )
       u1 = 2.0d0 * ( - w2*e1 - w2*w3*s1*c2*sx - w2*w3*c1*cx ) 
       tp = 2.0d0 * ( - w2*w3 + w2*w3*s1*c2*sx + w2*w3*c1*cx )
       up = 2.0d0 * ( - w1*w3 - w2*w3*sx*s1*c2 - w2*w3*cx*c1 + w3*p*c1 )
       s3 = 2.0d0 * (   w3*e2 - p*w3*c1 )
       s5 = 2.0d0 * ( e1*e2 + w3*p*c1 )+m1**2+m2**2 
    end if
    ...
    if (test(0)>1.d-6) print *," KIN_NN: test0 better be zero "
    if (test(1)>1.d-6) print *," KIN_NN: test1 better be zero "
    if (test(2)>1.d-6) print *," KIN_NN: test2 better be zero "
    if (test(3)>1.d-6) print *," KIN_NN: test3 better be zero "
    if (test(4)>1.d-6) print *," KIN_NN: test4 better be zero "
    if (test(5)>1.d-6) print *," KIN_NN: test5 better be zero "
...
! ------------------------------
  subroutine COUPLING_NN(s,m1,m2,iq,Cs,Ct,Cl,Cr,Cv,mx,iout,mkraemer)
    ...
    t3(iq)  = dble(iq) /2.0d0                                                  ! quark quantum numbers 
    qq(iq)  = 2.0d0/3.0d0 + ( dble(iq) - 1.0d0 ) /2.0d0

    i = (0.0d0,1.0d0) 

    sw = sqrt( 1.0d0 - mw**2/mz**2 )
    sw2 = sw**2
    cw2 = 1.0d0 - sw2
    cw  = sqrt(cw2)
    ...
    if ((ipart1<5).and.(ipart2<5)) then                                            ! NN case 
       ...
       v2  = - ( t3(iq) - 2.0d0 * sw2 * qq(iq)   )/(2.0d0*sw*cw)                 ! zqq couplings 
       a2  = -   t3(iq)                         /(2.0d0*sw*cw) 

       v1w = 0.0d0                                                             ! nnp coupling
       v2w = -i*aimag(zz(ipart1,3)*zzc(ipart2,3)-zz(ipart1,4)*zzc(ipart2,4))/(2.0d0*sw*cw) ! nnz couplings [higgsinos]
       a2w = -   real(zz(ipart1,3)*zzc(ipart2,3)-zz(ipart1,4)*zzc(ipart2,4))/(2.0d0*sw*cw)  

       Clx(1) = (zz(ipart1,1)*sw*(qq(iq)-t3(iq))+zz(ipart1,2)*cw*t3(iq))/(sqrt(2.0d0)*sw*cw) ! nqs coupings [gauginos]
       Clx(2) = (zz(ipart2,1)*sw*(qq(iq)-t3(iq))+zz(ipart2,2)*cw*t3(iq))/(sqrt(2.0d0)*sw*cw) 
       Crx(1) =  zzc(ipart1,1)*sw*qq(iq)                                /(sqrt(2.0d0)*sw*cw) 
       Crx(2) =  zzc(ipart2,1)*sw*qq(iq)                                /(sqrt(2.0d0)*sw*cw) 
       Clx(3:4) = Clx(1:2)
       Crx(3:4) = Crx(1:2) 
       ...
       v2w = (2.0d0*delta_ij*(sw2-cw2)-uu(ic1,1)*uu(ic2,1)-vv(ic1,1)*vv(ic2,1))/(4.0d0*sw*cw)  ! ccz couplings 
       a2w = (                    uu(ic1,1)*uu(ic2,1)-vv(ic1,1)*vv(ic2,1))/(4.0d0*sw*cw)

       Clx(1) = uu(ic1,1)/(2.0d0*sw)                      ! cqs coupings [gauginos]
       Clx(2) = uu(ic2,1)/(2.0d0*sw)                      ! only u quarks in fermion number conserving vertices
       Clx(3) = vv(ic1,1)/(2.0d0*sw)
       Clx(4) = vv(ic2,1)/(2.0d0*sw)                      ! only d quarks in fermion number violating vertices 
       Crx(1:4) = (0.0d0,0.0d0)                             ! only left handed su(2) doublets 
       ...
       v1  = 0.0d0                                                                ! pqq coupling
       v2  = - 1.0d0 /(2.0d0*sqrt(2.0d0)*sw)                                          ! wqq coupling
       a2  = - 1.0d0 /(2.0d0*sqrt(2.0d0)*sw) 

       v1w = 0.0d0                                                                ! pcn coupling 
       v2w = (  sqrt(2.0d0)*( zz(in,2)*uu(ic,1)+zzc(in,2)*vv(ic,1) )           & 
              + zz(in,3)*uu(ic,2) - zzc(in,4)*vv(ic,2) )/(2.0d0*sqrt(2.0d0)*sw)     ! wcn couplings
       a2w = (  sqrt(2.0d0)*( zzc(in,2)*vv(ic,1)-zz(in,2)*uu(ic,1) )           &
              - zz(in,3)*uu(ic,2) - zzc(in,4)*vv(ic,2) )/(2.0d0*sqrt(2.0d0)*sw)
...
!               quantum numbers here adjusted for NC+ production: neutralino in t(u) channel coupled to d(u) quark
       Clx(1) = uu(ic,1)/(2.0d0*sw)                                                          ! t-channel chargino coupling
       Clx(2) = ( zz(in,1)*sw*( qq(-1)-t3(-1) ) + zz(in,2)*cw*t3(-1) )/(sqrt(2.0d0)*sw*cw)   ! t-channel neutralino coupling
       Clx(3) = vv(ic,1)/(2.0d0*sw)                                                          ! u-channel chargino coupling
       Clx(4) = ( zz(in,1)*sw*( qq(+1)-t3(+1) ) + zz(in,2)*cw*t3(+1) )/(sqrt(2.0d0)*sw*cw)   ! u-channel neutralino coupling

       Crx(1) = (0.0d0,0.0d0)
       Crx(2) =  zzc(in,1)*sw* qq(-1)/(sqrt(2.0d0)*sw*cw) 
       Crx(3) = (0.0d0,0.0d0)
       Crx(4) =  zzc(in,1)*sw* qq(+1)/(sqrt(2.0d0)*sw*cw) 
    ...
    Cs(1) = (v1*v1w)**2 + s * 2.0d0 * real( v1*v1w*v2*v2w )/(s-mx**2)         &  ! wim's conventions 
           + s**2*( v2**2+a2**2 )*( abs(v2w)**2+abs(a2w)**2 ) /(s-mx**2)**2    ! [v1,v2,v1w,a2 real]
    Cs(2) = (v1*v1w)**2 + s * 2.0d0 * real( v1*v1w*v2*v2w )/(s-mx**2)         &
           + s**2*( v2**2+a2**2 )*( abs(v2w)**2-abs(a2w)**2 ) /(s-mx**2)**2
    Cs(3) = ( 2.0d0 * real( v1*v1w*a2*a2w ) + s * 2.0d0*a2*v2                   &
           * 2.0d0*real( a2w*conjg(v2w) ) /(s-mx**2)         )/2.0d0
    Cs(4) = 0.0d0 
    ...
    do i1=1,4
       if ( abs(aimag(Cl(i1))) > 1.d-8 ) then
          print*, " COUPLING_NN: problem with complex couplings ",i1,Cl(i1)
          call HARD_STOP
       end if
       if ( abs(aimag(Cr(i1))) > 1.d-8 ) then
          print*, " COUPLING_NN: problem with complex couplings ",i1,Cr(i1)
          call HARD_STOP
       end if
    end do

    only_u = ( 1.0d0 + iq )/2.0d0                                                ! incoming quarks
    only_d = ( 1.0d0 - iq )/2.0d0
    ...
    mkraemer(12:28) = 2.0d0 * sqrt(pi) * mkraemer(12:28)             ! rescaling with 2 and pi 
    
    Cs(1:3) = 16.0d0 * pi**2    * Cs(1:3)                            ! rescaling g^2 -> alpha
    Ct(1:4) =  4.0d0 * pi       * Ct(1:4)
    Cl(1:4) =  2.0d0 * sqrt(pi) * Cl(1:4)
    Cr(1:4) =  2.0d0 * sqrt(pi) * Cr(1:4)
