...
! ------------------------------
  function IFCT_HH_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha,alpha_s,mu_tgb,nlo
    ...
    if (isca==0) then                                          ! renormalization/factorization scale, factorization scale low 
       qf = scafac  * (m1+m2)/4.0d0
       qr = scafac  * (m1+m2)/2.0d0
    else if (isca==1) then
       qf = scafac * sqrt(s)/4.0d0
       qr = scafac * sqrt(s)
    end if

    if (qf < 30.0d0) qf = 30.0d0                                   ! too small scales are not good
    if (qr < 30.0d0) qr = 30.0d0

    if (iscaling==0) then                                      ! nlo factor [always nlo alpha_s]
       if (ii > 0) then
          alpha_s = ALPHAS(qr,2)
          nlo     = 4.0d0 * pi * alpha_s
       else
          nlo = 0.0
       end if
    ...
    if ((s>(mt+m2)**2).and.(mt>m1)) theta_s4 = 1.0d0             ! intermediate top
    if ((s>(mt+m1)**2).and.(mt>m2)) theta_s3 = 1.0d0             ! intermediate top

    del_s4 = eps_sli * min(m1**2,m2**2)                      & ! unit is m^2
                   * (1.0d0-(m1+m2)**2/s)*(1.0d0-(m1-m2)**2/s)     ! rescale the s4 cutoff 
    if (ii/=4) del_s4 = 0.0d0

    s4p    = 0.0d0                                                               ! only for real corrections 

    select case (ii)                                                           ! all the phase spaces 
    case(-1,0,1,2,3)                                                           ! born_lo, born_nlo, virt

       beta   = sqrt(1.0d0-(m1+m2)**2/s)                       &                 ! t2 integration
               *sqrt(1.0d0-(m1-m2)**2/s)      
    ...
    case(4)                                                                    ! real(qb)
       ...
       beta   = ((1.0d0-del_s4/s)**2-(m1+m2)**2/s)              &                ! t2 integration shifted 
               *((1.0d0-del_s4/s)**2-(m1-m2)**2/s)                    
       if (beta>=0.0d0) then 
          beta = sqrt(beta)
       else 
...
!       s4_jac = s4 * log(s4p/s4m)
    ...
    case(5)                                                                    ! crossed(qg), no os subtraction 
       ...
       s3_1p    = s + m1**2 - m2**2 - 2.0d0 * sqrt(s*m1**2)
       s3_1     = var(5) * (s3_1p-s3_1m) + s3_1m
       s3_1_jac = s3_1p-s3_1m

       beta1_1    = (s-s3_1-m2**2+m1**2)**2 - 4.0d0*m1**2*s 
       ...
       s4_1m    = s3_1/2.0d0/(s3_1+m2**2) * (s - s3_1 - m1**2 - m2**2 - beta1_1)
       s4_1p    = s3_1/2.0d0/(s3_1+m2**2) * (s - s3_1 - m1**2 - m2**2 + beta1_1)
       s4_1     = var(6) * (s4_1p-s4_1m) + s4_1m
       s4_1_jac = s4_1p-s4_1m
       
       beta2_1    = (s-s4_1-m1**2+m2**2)**2 - 4.0d0*s*m2**2  
       ...
       t2_1m    = -1.0d0/2.0d0 * ( s - s4_1 - m1**2 + m2**2 + beta2_1 )
       t2_1p    = -1.0d0/2.0d0 * ( s - s4_1 - m1**2 + m2**2 - beta2_1 )
       t2_1     = var(7) * (t2_1p-t2_1m) + t2_1m 
       t2_1_jac = t2_1p-t2_1m 
       
       t2_1_jac = t2_1_jac * 2.0d0/s4_1*(s4_1+m1**2) / beta2_1                   ! including the over-all jacobian
    ...
    case(6)                                                                    ! on-shell(qg)
       ...
       beta1_2    = (s-s3_2-m2**2+m1**2)**2 - 4.0d0*m1**2*s 
       ...
       s4_2m    = s3_2/2.0d0/(s3_2+m2**2) * (s - s3_2 - m1**2 - m2**2 - beta1_2)
       s4_2p    = s3_2/2.0d0/(s3_2+m2**2) * (s - s3_2 - m1**2 - m2**2 + beta1_2)
       s4_2     = var(4) * (s4_2p-s4_2m) + s4_2m
       s4_2_jac = s4_2p-s4_2m
       
       beta2_2    = (s-s4_2-m1**2+m2**2)**2 - 4.0d0*s*m2**2  
       ...
       t2_2m    = -1.0d0/2.0d0 * ( s - s4_2 - m1**2 + m2**2 + beta2_2 )
       t2_2p    = -1.0d0/2.0d0 * ( s - s4_2 - m1**2 + m2**2 - beta2_2 )
       t2_2     = var(5) * (t2_2p-t2_2m) + t2_2m 
       t2_2_jac = t2_2p-t2_2m 
       
       t2_2_jac = t2_2_jac * 2.0d0/s4_2*(s4_2+m1**2) / beta2_2                   ! including the over-all jacobian
    ...
    case(7,8)                                                                  ! crossed(gb) including on-shell
       ...
       if ( (ii==8).and.(theta_s4==0.0d0) ) then 
          dsig = 0.0d0
          return
       end if
       ...
       beta   = sqrt( dabs( (s-s4-m1**2+m2**2)**2 - 4.0d0*s*m2**2 ) )            ! absolute value because of numerics
       ...
       if (theta_s4==1.0d0) then                                                 ! restricted phase space for theta_s4=1 
          ...
          beta_1   = sqrt( (s-s4_1-m1**2+m2**2)**2 - 4.0d0*s*m2**2 )
          t2_1m    = -1.0d0/2.0d0 * ( s - s4_1 - m1**2 + m2**2 + beta_1 )
          t2_1p    = -1.0d0/2.0d0 * ( s - s4_1 - m1**2 + m2**2 - beta_1 )
    ...
    massin(26) = 0.0d0                                           ! gamma/m of s channel particles in breit wigner
    ...
    if ( (m1+m2) < mz )  C(9)  = 0.0d0                           ! switch off s-channel couplings  
    if ( (m1+m2) < mh1 ) C(10) = 0.0d0
    if ( (m1+m2) < mh2 ) C(11) = 0.0d0
    ...
    if (iscaling==0)                                         &  ! what is missing from the matrix elements 
        dsig = dsig * 4.0d0/(m1+m2)**2 * (4.D0*pi*alpha)**2 
...
! ------------------------------
  subroutine COUPLING_HH(qr,C)
    ...
    t3 = -0.5d0                                                  ! bottom quark quantum numbers 
    qq = -1.0d0/3.0d0

    sw = sqrt( 1.0d0 - mw**2/mz**2 )                             ! weak mixing angle 
    sw2 = sw**2
    cw2 = 1.0d0 - sw2
    cw  = sqrt(cw2)
    c2w = cw**2-sw**2
    ...
    C(1:20) = 0.0d0                                              ! initialize arrays
    ...
    C(8)  = -e                                                 ! ssp 
    C(9)  = -g*c2w/2.0d0/cw                                      ! ssz 
    C(10) = -g*( mw*sbma + mz/2.0d0/cw*c2b*sbpa )                ! lambda1 (checked with ME)
    C(11) = -g*( mw*cbpa - mz/2.0d0/cw*c2b*cbma )                ! lambda2 (checked with ME)
