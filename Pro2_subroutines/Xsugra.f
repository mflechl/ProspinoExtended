...
c    ms,mc,i_mb,i_mt,i_mtau,i_mz,i_mw,i_gf,...  fixed inside the program           c
c                                                                      c
...
c                                                                      c
c                       lowmass(91) trilinear higgs coupling lai_mbda(1) c
c                       .......                                        c
c                       lowmass(97) lai_mbda(7)                          c
c                                                                      c
c                       lowmass(99)                                    c
c                                                                      c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &       ,XITLA,RUNM_ORIG,B_SPECIAL,lam_qcd,lai_mbda(1:7)
     &       ,m0(3),m12,a0,sgnmu,ms,mc,gmas(3)
     &       ,alsi_mz,cb,cbb,sb,acc,sw02,sw2,al1,al2,al3
     &       ,b(3),mx,alpx,cof(3),cofq(2),alsq,q
     &       ,qq(2),dd,cgl(2),mer,mel,mnl,mur,mdr,mul,mdl,ri_mt,ri_mb
     ...
     &       ,i_pi,i_mz,i_mw,sw,swos,i_gf,alpi_mz,i_mb
     &       ,i_mt,i_mtau,tb,default
...
c               weak coupling sector given for 1-3-unification 
      if (nuni.eq.13) then      
         alsi_mz = 0.118D0
c               everything the on-shell and i_gf scheme  
         i_gf    = 1.16639D-5
         sw    = sqrt( 1.D0 - i_mw**2/i_mz**2 )
         swos  = sw
         alpi_mz = i_gf    * sqrt(2.D0) * sw**2 * i_mw**2 / i_pi
      elseif (nuni.eq.12) then 
c               everything in the msbar scheme,
c               except for low energy masses 
         alpi_mz = 1.D0/127.9D0
         if (ncomphep.eq.0) then 
            sw    = sqrt( 0.2315D0 ) 
            swos  = sqrt( 1.D0 - i_mw**2/i_mz**2 )
         else if (ncomphep.eq.1) then
            sw    = 0.4740D0
            swos  = sw
            print*,' Xsugra.f: sw set a la CompHEP '
         end if
         i_gf    = alpi_mz / sqrt(2.D0) / sw**2 / i_mw**2 * i_pi 
      else 
...
c               the low-scale gauge couplings [cms 7.17]
      al1 = alpi_mz * (1.D0-sw02)/sw02 /(1.D0-sw**2)

      if (nuni.eq.13) then 
         al3 = alsi_mz
c               the gut scale [cms 7.14]
         mx = i_mz * dexp(2.D0*i_pi/(b(3)-b(1))*(1.D0/al1-1.D0/al3))
c               the unified gauge coupling alpha(mx) [cms 7.14] 
         alpx = al3/(1.D0 + al3*b(3)/(4.D0*i_pi) * log(mx**2/i_mz**2))
      else if (nuni.eq.12) then 
         al2 = alpi_mz/sw**2 
c               the gut scale [cms 7.14]
         mx = i_mz * dexp(2.D0*i_pi/(b(2)-b(1))*(1.D0/al1-1.D0/al2))
c               the unified gauge coupling alpha(mx) [cms 7.14] 
         alpx = al2/(1.D0 + al2*b(2)/(4.D0*i_pi) * log(mx**2/i_mz**2))
      end if 

c               the different low-scale gauge couplings 
      do n=1,3 
       cof(n) = 1.D0/(1.D0 + alpx*b(n)/(4.D0*i_pi) * log(i_mz**2/mx**2))
      end do

c               the alpha_s value for qcd type different masses 
      alsq = cof(3) * alpx

c               the improved lai_mbda_qcd value at q=i_mz for ALPHAS routine
      nloop   = 2
      q       = i_mz
...
c               the running top and bottom mass
      ri_mt = RUNM_ORIG(i_mt,6) * (1.D0 + 8.D0/3.D0 * alpi_mz**2)
      ri_mb = RUNM_ORIG(i_mt,5) * (1.D0 + 8.D0/3.D0 * alpi_mz**2)

c               approximate yukawa couplings [dm 25]
      xt = (ri_mt/150.D0/sb)**2 * 
     &     (  0.9D0*m0(2)**2 + 2.1D0*m12**2 
     &      + (1.D0-(ri_mt/190.D0/sb)**3) * (0.24D0*a0**2 + a0*m12) )
      xb = (ri_mb/150.D0/cb)**2 * 
     &     (  0.9D0*m0(2)**2 + 2.1D0*m12**2
     .      + (1.D0-(ri_mb/190.D0/cb)**3) * (0.24D0*a0**2 + a0*m12) )
...
c               approximate stop trilinear coupling [dm 26]
      at = (  a0 * (1.D0-(ri_mt/190.D0/sb)**2)
     &      + m12 * (3.47D0-1.9D0*(ri_mt/190.D0/sb)**2) )
...
c               approximate higgsino mass parameter [dm 9a]
      mu2 = ( mh22*sb**2 - mh12*cb**2 )/cbb - i_mz**2/2.D0
      if (mu2.gt.0.D0) then 
         mu = sgnmu * sqrt(mu2)
      else 
...
c               stop mass matrix [dm 12a, thesis 1.8]
      m2stl = mql3**2 + ri_mt**2 + (0.5D0-2.D0/3.D0*sw2) *dd
      m2str = mur3**2 + ri_mt**2 +        2.D0/3.D0*sw2  *dd
      mlrt  = -ri_mt * (at+mu/tb)
...
c               sbottom mass matrix [dto]
      m2sbl = mql3**2 + ri_mb**2 + (-0.5D0+1.D0/3.D0*sw2) *dd
      m2sbr = mdr3**2 + ri_mb**2 -         1.D0/3.D0*sw2  *dd
      mlrb  = -ri_mb * (ab+mu*tb)
...
c               stau mass matrix [dm 12b]
      m2sel = mel3**2 + i_mtau**2 + (-0.5D0+sw2) *dd
      m2ser = mer3**2 + i_mtau**2 -         sw2  *dd
      m2sn3 = mnl3**2           +   0.5D0      *dd
      mlre  = -i_mtau * (al+mu*tb)
...
c               approximate gluino mass [dm 19] 
      eps = 1.d-2
      mg = m3 * ( 1.D0 + alsq/(4.D0*i_pi) * ( 15.D0 - 18.D0*log(m3/q)
     ...
     &           + B_SPECIAL(m3**2,mur3,i_mt,q**2)
     &           + B_SPECIAL(m3**2,mql3,i_mt,q**2)
     &           + B_SPECIAL(m3**2,mdr3,i_mb,q**2)
     &           + B_SPECIAL(m3**2,mql3,i_mb,q**2)
     &    ))

c               all the gaugino masses and mixing matrices 
      call GAUGINO(tb,i_mw,i_mz,swos,mu,m1,m2,mchar,mneut,bw,pz,uu,vv)

c               the higgs sector a la carena et al. or to leading order
      if (nlo_h.eq.0) then 
         call BORNH(ma,tb,i_mw,i_mz,mhl,mhh,mhc,sa,ca)
         do n=1,7,1
            lai_mbda(n) = 0.D0
         end do
      else if (nlo_h.eq.1) then 
         mchi = mchar(2)
         call SUBH(ma,tb,i_mw,i_mz,swos,i_gf,i_mt
     &            ,mql3,mur3,mdr3,at,ab,mu,mchi,default
     &            ,mhl,mhh,mhc,sa,ca,tanba,lai_mbda)
      end if 
c               check the diagonalization of the mass matrices 
      call DIAGONALIZE(tb,i_mw,i_mz,swos,i_mt,alpi_mz,i_mtau
...
c               translate the output 
      ...
      lowmass(91) = lai_mbda(1)
      lowmass(92) = lai_mbda(2)
      lowmass(93) = lai_mbda(3)
      lowmass(94) = lai_mbda(4)
      lowmass(95) = lai_mbda(5)
      lowmass(96) = lai_mbda(6)
      lowmass(97) = lai_mbda(7)
...
c           mass and mixing matrices; i.e. output bw,pz,uu,vv 
      subroutine GAUGINO(tb,i_mw,i_mz,sw,mu,m1,m2,mc,mn,bw,pz,uu,vv)
...
c               neutralino mass matrix similar to b.kileng {-p,q,r}
      xc2 = m1*m2 - i_mz**2 - mu**2 - 3.D0/8.D0*(m1+m2)**2
      xc3 = - 1.D0/8.D0*(m1+m2)**3
     &      + 1.D0/2.D0*(m1+m2)*(m1*m2-i_mz**2-mu**2)
     &      + (m1+m2)*mu**2
     &      + (m1*cw**2+m2*sw**2)*i_mz**2
     &      - mu*i_mz**2*sbb
      xc4 =   (m1*cw**2+m2*sw**2)*mu*i_mz**2*sbb
     &      - m1*m2*mu**2
     &      + 1.D0/4.D0*(m1+m2)*( (m1+m2)*mu**2
     &                           +(m1*cw**2+m2*sw**2)*i_mz**2
     &                           -mu*i_mz**2*sbb )
     &      + 1.D0/16.D0*(m1+m2)**2*(m1*m2-i_mz**2-mu**2)
     &      - 3.D0/256.D0*(m1+m2)**4
...
c               bw basis mixing matrix yz(4,4) [thesis A.8]
     ...
     &              -i_mz**2*sb*cb * ( (m1-m2)*cw**2+m2-xmn(i)))
     &             /i_mz/(m2-xmn(i))/sw/(mu*cb+xmn(i)*sb)
         zx(i,4) = (-xmn(i) * (m2-xmn(i)) * (m1-xmn(i))
     &              -i_mz**2*cb*cb * ( (m1-m2)*cw**2+m2-xmn(i)))
     &             /i_mz/(m2-xmn(i))/sw/(mu*cb+xmn(i)*sb)
...
c               the charginos [kileng 3.116 with sb<->cb]
      sx = (m2**2+mu**2)/2.D0 + i_mw**2
      tx = (m2**2-mu**2)**2/4.D0 + i_mw**4*cbb**2  
     &    + i_mw**2*( m2**2 + mu**2 + 2.D0*mu*m2*sbb )
...
c               mixing angles [kileng 3.118]
      c2u = ( m2**2 - mu**2 - 2.D0*i_mw**2*cbb )/(mc(2)**2-mc(1)**2)
      c2v = ( m2**2 - mu**2 + 2.D0*i_mw**2*cbb )/(mc(2)**2-mc(1)**2)
...
c               mixing matrices 
      ...
      vvx(1,1) =  cv 
      vvx(1,2) =  sv 
      vvx(2,1) = -sv * sign(1.D0,mu*m2-i_mw**2*sbb)
      vvx(2,2) =  cv * sign(1.D0,mu*m2-i_mw**2*sbb)

c               one last sign to be fixed 
      cm(1,1) = m2
      cm(1,2) = sqrt(2.D0)*i_mw*sb
      cm(2,1) = sqrt(2.D0)*i_mw*cb
      cm(2,2) = mu
...
c ----------------------------------------------------------------------
c        iteration routine to determine improved lai_mbda's
      real*8 function XITLA(nloop,als_fix,q,acc)

      implicit none 

      integer nloop,nf,n
      real*8  als_fix,q,acc,aa,bb,als,xit,i_pi,a,b,x,lam,diff

c               the over-all prefactor 1/beta0 as usual 
      aa(nf,i_pi)= 12.D0*i_pi / (33.D0 - 2.D0*dble(nf))
c               beta1 as usual
      bb(nf,i_pi)= (153.D0-19.D0*dble(nf))/(33.D0-2.D0*dble(nf))
     &           /(2.D0*i_pi)

c               alpha_s in terms of x=log(q^2/lai_mbda^2)
      als(nf,x) = aa(nf,i_pi)/x*(1.D0-bb(nf,i_pi)*aa(nf,i_pi)*log(x)/x )
...
c               the prefactors in the calculation       
      nf = 5
      i_pi = 4.D0 * atan(1.D0)
      a = aa(nf,i_pi)/als_fix
      b = bb(nf,i_pi)*als_fix

c               the starting value in leading order 
      lam = q * dexp(-aa(nf,i_pi)/als_fix/2.D0)
...
c               iteration of lai_mbda using initial value 
         x = log(q**2/lam**2)
         lam = q * dexp(-xit(a,b,x)/2.D0)
      
c               improved value for lai_mbda
         x = log(q**2/lam**2) 
...
c                in case the accuracy is not reached 
      print*,'Xsugra.f:  iteration for lam_qcd stopped, lai_mbda =',lam
      XITLA = lam
...
c               all variables are input 
      subroutine DIAGONALIZE(tb,i_mw,i_mz,sw,i_mt,alpi_mz,i_mtau
     ...
     &       ,sw2,ri_mt,ri_mb,cbb,at,ab,al,mu,RUNM_ORIG
     ...
     &       ,i_mz,i_mw,sw,alpi_mz,i_mt,i_mtau,tb,del_check
...
c               chargino mass matrix [thesis A.11]
      cm(1,1) = m2
      cm(1,2) = sqrt(2.D0)*i_mw*sbet
      cm(2,1) = sqrt(2.D0)*i_mw*cbet
      cm(2,2) = mu
...
c               stop mass matrix [dm 12a]
      mstl2 = mul**2 + (0.5D0-2.D0/3.D0*sw2) * i_mz**2*cbb
      mstr2 = mur**2 +        2.D0/3.D0*sw2  * i_mz**2*cbb
      st(1,1) = mstl2 + ri_mt**2
      st(1,2) = -ri_mt * (at+mu/tb)
      st(2,1) = -ri_mt * (at+mu/tb)
      st(2,2) = mstr2 + ri_mt**2
...
c               sbottom mass matrix [dto]
      msbl2 = mdl**2 + (-0.5D0+1.D0/3.D0*sw2) * i_mz**2*cbb
      msbr2 = mdr**2 -         1.D0/3.D0*sw2  * i_mz**2*cbb
      sb(1,1) = msbl2 + ri_mb**2
      sb(1,2) = -ri_mb * (ab+mu*tb)
      sb(2,1) = -ri_mb * (ab+mu*tb)
      sb(2,2) = msbr2 + ri_mb**2
...
c               stau mass matrix [dm 12b]
      msll2 = mel**2 + (-0.5D0+sw2) * i_mz**2*cbb
      mslr2 = mer**2 -         sw2  * i_mz**2*cbb
      sl(1,1) = msll2 + i_mtau**2
      sl(1,2) = -i_mtau * (al+mu*tb)
      sl(2,1) = -i_mtau * (al+mu*tb)
      sl(2,2) = mslr2 + i_mtau**2
...
c$$$      write(6,*)
c$$$      write(6,'(a6,/,2(2(f15.6,1x),/))')'i_mt0:',((st(i,j),j=1,2),i=1,2)
c$$$      write(6,*)
c$$$      write(6,'(a6,/,2(2(f12.5,1x),/))')'i_mt:',
c$$$     .                            ((sqrt(abs(dt(i,j))),j=1,2),i=1,2)
c$$$      write(6,*)
c$$$      write(6,'(a6,/,2(2(f14.8,1x),/))')'zt:',((zt(i,j),j=1,2),i=1,2)
c$$$      write(6,*)
c$$$      write(6,'(a6,/,2(2(f15.6,1x),/))')'i_mb0:',((sb(i,j),j=1,2),i=1,2)
c$$$      write(6,*)
c$$$      write(6,'(a6,/,2(2(f14.5,1x),/))')'i_mb:',
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine SUBH(ma,tb,i_mw,i_mz,sw,i_gf,i_mt
     ...
     &      ,ri_mb,ri_mt,tq,tu,td,sinb,cosb,g1,g2,g3,hu,hd
     &      ,tp,tpd,tdp,tdpd,dlai_mbda1,dlai_mbda2,dlai_mbda3,dlai_mbda4
     &      ,lai_mbda(1:7),vh(2,2),m2(2,2),m2p(2,2),mssusy,tchar
     &      ,deltal12,deltal3p4,deltam112,deltam122,deltam222
     &      ,trm2p,detm2p,mhl2,mhh2,mhc2,cos2alpha
     &      ,RUNM_ORIG
     &      ,i_pi,i_mz,i_mw,sw,i_gf,i_mt,tb,default

      external RUNM_ORIG

      i_pi = 4.D0 * atan(1.D0)

      v   = 1/sqrt(2.D0*sqrt(2.D0)*i_gf)
      sw2 = sw**2 
      cw2 = 1.D0-sw2 

      alpha2  = (2.D0*i_mw/v/sqrt(2.D0))**2/4.D0/i_pi
      alpha1  = alpha2*sw2/cw2

c               alphas a la si_pira has to be initialized 
      alpha3  = ALPHAS(i_mt,2)

c               running quark masses used 
      ri_mb = RUNM_ORIG(i_mt,5)
      ri_mt = RUNM_ORIG(i_mt,6)

      tq = log((mq**2  + i_mt**2)/i_mt**2)
      tu = log((mur**2 + i_mt**2)/i_mt**2)
      td = log((md**2  + i_mt**2)/i_mt**2)
...
c               tan(beta) reset for 
      if (ma.gt.i_mt)
     *     tanba = tb*(1.D0-3.D0/32.D0/i_pi**2*
     *             (ri_mt**2/v**2/sinb**2-ri_mb**2/v**2/cosb**2)*
     *             log(ma**2/i_mt**2))
      if (ma.le.i_mt) tanbt = tanba

c               sin/cos(beta) set at scale i_mt 
      sinb = tanbt/sqrt(1.D0 + tanbt**2)
      cosb = 1.D0/sqrt(1.D0 + tanbt**2)

c               gauge couplings at scale i_mz
      g1 = sqrt(alpha1*4.D0*i_pi)
      g2 = sqrt(alpha2*4.D0*i_pi)
      g3 = sqrt(alpha3*4.D0*i_pi)

c               yukawa couplings at scale i_mt 
      hu = ri_mt/v/sinb
      hd = ri_mb/v/cosb
      ...
      if (mq.gt.md) then 
         dlai_mbda1 = 6.D0/96.D0/i_pi**2*hd**2*g1**2*tpd
      else if (mq.le.md) then 
         dlai_mbda1 = 3.D0/32.D0/i_pi**2*hd**2*(g1**2/3.D0+g2**2)*tpd
      end if 
         
      if(mq.gt.mur) then 
         dlai_mbda2 = 12.D0/96.D0/i_pi**2*hu**2*g1**2*tp
      else if (mq.le.mur) then 
         dlai_mbda2 =  3.D0/32.D0/i_pi**2*hu**2*(-g1**2/3.D0+g2**2)*tp
      end if 
      ...
      if (mq.gt.md) then 
         dlai_mbda3 = -1.D0/32.D0/i_pi**2*hd**2*g1**2*tpd
      else if (mq.le.md) then 
         dlai_mbda3 =  3.D0/64.D0/i_pi**2*hd**2*(g2**2-g1**2/3.D0)*tpd
      end if 

      if (mq.gt.mur) then 
         dlai_mbda3 =dlai_mbda3-1.D0/16.D0/i_pi**2*hu**2*g1**2*tp
      else if (mq.le.mur) then 
         dlai_mbda3 =dlai_mbda3
     &          +3.D0/64.D0/i_pi**2*hu**2*(g2**2+g1**2/3.D0)*tp
      end if 

      if (mq.lt.mur) then 
         dlai_mbda4 = -3.D0/32.D0/i_pi**2*hu**2*g2**2*tp
      end if 

      if (mq.lt.md) then
         dlai_mbda4 = dlai_mbda4 - 3.D0/32.D0/i_pi**2*g2**2*hd**2*tpd
      end if 

c               all the usual lai_mbdas, as defined in kane's book 
      lai_mbda(1) = ((g1**2+g2**2)/4.D0)
     &           * (1.D0-3.D0*hd**2*(tpd + tdpd)/8.D0/i_pi**2)
     &        + (3.D0*hd**4.D0/16.D0/i_pi**2) * tpd 
     &           * (1.D0 + (3.D0*hd**2/2.D0 + hu**2/2.D0 - 8.D0*g3**2)
     &                      * (tpd + 2.D0*tdpd)/16.D0/i_pi**2          )
     &        + (3.D0*hd**4.D0/8.D0/i_pi**2) * tdpd
     &           * (1.D0  + (3.D0*hd**2/2.D0 + hu**2/2.D0 - 8.D0*g3**2)
     &                       * tdpd/16.D0/i_pi**2                      )
     &        + dlai_mbda1 

      lai_mbda(2) = ((g1**2 + g2**2)/4.D0)
     &           *(1.D0-3.D0*hu**2*(tp + tdp)/8.D0/i_pi**2)
     &        + (3.D0*hu**4.D0/16.D0/i_pi**2) * tp
     &           *(1.D0 + (3.D0*hu**2/2.D0 + hd**2/2.D0 - 8.D0*g3**2) 
     &                     * (tp + 2.D0*tdp)/16.D0/i_pi**2             )
     &        + (3.D0*hu**4.D0/8.D0/i_pi**2) * tdp
     &           * (1.D0 + (3.D0*hu**2/2.D0 + hd**2/2.D0 - 8.D0*g3**2)
     &                      * tdp/16.D0/i_pi**2                        )
     &        + dlai_mbda2 

      lai_mbda(3) = ((g2**2 - g1**2)/4.D0)
     &           *(1.D0-3.D0*hu**2*(tp + tdp)/16.D0/i_pi**2 
     &                - 3.D0*hd**2*(tpd + tdpd)/16.D0/i_pi**2) 
     &        + dlai_mbda3 

      lai_mbda(4) = (- g2**2/2.D0)
     &           *(1.D0-3.D0*(hu**2)*(tp + tdp)/16.D0/i_pi**2
     &                 -3.D0*(hd**2)*(tpd + tdpd)/16.D0/i_pi**2) 
     &        + dlai_mbda4
...
c               the cp even mass matrix 
      m2(1,1) = 2.D0*v**2*(  lai_mbda(1)*cosb**2 
     &                     + 2.D0*lai_mbda(6)*cosb*sinb 
     &                     + lai_mbda(5)*sinb**2) 
     &          + ma**2*sinb**2
      m2(2,2) = 2.D0*v**2*(  lai_mbda(5)*cosb**2 
     &                     + 2.D0*lai_mbda(7)*cosb*sinb 
     &                     + lai_mbda(2)*sinb**2) 
     &          + ma**2*cosb**2
      m2(1,2) = 2.D0*v**2*(  lai_mbda(6)*cosb**2 
     &                     + ( lai_mbda(3) + lai_mbda(4) )*cosb*sinb
     &                     + lai_mbda(7)*sinb**2) 
     &          - ma**2*sinb*cosb
      m2(2,1) = m2(1,2)

c               contribution from light charginos/neutralinos
      mssusy=sqrt(0.5D0*(mq**2+mur**2)+i_mt**2)
      if (mchi.le.mssusy) then 
         if (mchi.lt.i_mt) mchi=i_mt
         
         tchar = log(mssusy**2/mchi**2)

         deltal12  = ( 9.D0/64.D0 /i_pi**2*g2**4
     &               + 5.D0/192.D0/i_pi**2*g1**4 )*tchar
         deltal3p4 = ( 3.D0/64.D0 /i_pi**2*g2**4
     &               + 7.D0/192.D0/i_pi**2*g1**4
     &               + 4.D0/32    /i_pi**2*g1**2*g2**2 )*tchar
      ...
      call GFUN(ma,tb,i_mw,i_mz,sw,i_gf,
     &          i_mt,mq,mur,md,au,ad,mu,default,vh)
...
c               charged higgs mass !!!!!!!!!!!!!!!!
      mhc2 = ma**2 + (lai_mbda(5)-lai_mbda(4)) * v**2
...
c               output: vh
      subroutine GFUN(ma,tanb,i_mw,i_mz,sw,i_gf,i_mt
     ...
     &      ,ri_mb,ri_mt,mst,msusyt,msb,msusyb,tt,tb,ht,htst,hb
     &      ,bt2,bb2,al2,al1,i_mt4,i_mt2,i_mb4,i_mb2
     ...
     &      ,RUNM_ORIG
     &      ,i_pi,i_mz,i_mw,sw,i_gf,i_mt,tanb,default
...
c               WHY NO RUNNING TOP MASS ???
      mst = dmax1(mq,mur)
      msusyt = sqrt(mst**2  + i_mt**2)

      msb = dmax1(mq,md)
      msusyb = sqrt(msb**2 + ri_mb**2)

      tt = log(msusyt**2/i_mt**2)
      tb = log(msusyb**2/i_mt**2)
...
c               yukawa couplings using running masses 
      ...
      bt2   = -(8.D0*g32 - 9.D0*ht**2/2.D0 - hb**2/2.D0)/(4.D0*i_pi)**2
      bb2   = -(8.D0*g32 - 9.D0*hb**2/2.D0 - ht**2/2.D0)/(4.D0*i_pi)**2
      al2   = 3.D0/8.D0/i_pi**2 * ht**2
      al1   = 3.D0/8.D0/i_pi**2 * hb**2
      ...
      i_mt4 = ri_mt**4 * (1.D0 + 2.D0*bt2*tt - al2*tt)
      i_mt2 = sqrt(i_mt4)
      i_mb4 = ri_mb**4 * (1.D0 + 2.D0*bb2*tb - al1*tb)
      i_mb2 = sqrt(i_mb4)

      if(ma.gt.i_mt) then
         vi  = v* (1.D0 + 3.D0/32.D0/i_pi**2*htst**2*log(i_mt**2/ma**2))
         h1i = vi * cosba
         h2i = vi * sinba
         h1t = h1i * sqrt(sqrt((1.D0 
     &                 + 3.D0/8.D0/i_pi**2*hb**2*log(ma**2/msusyt**2))))
         h2t = h2i * sqrt(sqrt((1.D0
     &                 + 3.D0/8.D0/i_pi**2*ht**2*log(ma**2/msusyt**2))))
         h1b = h1i * sqrt(sqrt((1.D0
     &                 + 3.D0/8.D0/i_pi**2*hb**2*log(ma**2/msusyb**2))))
         h2b = h2i * sqrt(sqrt((1.D0
     &                 + 3.D0/8.D0/i_pi**2*ht**2*log(ma**2/msusyb**2))))
      else
         vi  =  v
         h1i = vi*cosb
         h2i = vi*sinb
         h1t = h1i * sqrt(sqrt((1.D0
     &               + 3.D0/8.D0/i_pi**2*hb**2*log(i_mt**2/msusyt**2))))
         h2t = h2i * sqrt(sqrt((1.D0
     &               + 3.D0/8.D0/i_pi**2*ht**2*log(i_mt**2/msusyt**2))))
         h1b = h1i * sqrt(sqrt((1.D0
     &               + 3.D0/8.D0/i_pi**2*hb**2*log(i_mt**2/msusyb**2))))
         h2b = h2i * sqrt(sqrt((1.D0
     &               + 3.D0/8.D0/i_pi**2*ht**2*log(i_mt**2/msusyb**2))))
      end if
...
c               d-terms
     ...
     &     + (.5D0-2.D0/3.D0*sw2) * log(stop1*stop2/(mq2+i_mt2))
     &     +       2.D0/3.D0*sw2  * log(stop1*stop2/(mur2+i_mt2))
      
      f1b = (mq2-md2)/(sbot12-sbot22)
     &               * (-.5D0+2.D0/3.D0*sw2) * log(sbot1/sbot2)
     &     + (-.5D0+1.D0/3.D0*sw2) * log(sbot1*sbot2/(mq2+i_mb2))
     &     -        1.D0/3.D0*sw2  * log(sbot1*sbot2/(md2+i_mb2))

      f2t = sqrt(i_mt2)*(au1-mu/tanbst)/(stop12-stop22)
     &                * (-.5D0*log(stop12/stop22)
     &     + (4.D0/3.D0*sw2-.5D0) * (mq2-mur2)/(stop12-stop22)
     &                            * g(stop12,stop22))

      f2b = sqrt(i_mb2)*(ad1-mu*tanbsb)/(sbot12-sbot22)
     &                * (.5D0*log(sbot12/sbot22)
     &     + (-2.D0/3.D0*sw2+.5D0) * (mq2-md2)/(sbot12-sbot22)
     &                             * g(sbot12,sbot22))

      vh3b(1,1) = i_mb4/(cosbb**2)
     &  * ( log(sbot1**2*sbot2**2/(mq2+i_mb2)/(md2+i_mb2)) 
     &     + 2.D0 * (ad1*(ad1-mu*tanbsb)/(sbot1**2-sbot2**2))
     &            * log(sbot1**2/sbot2**2))  
     &     + i_mb4/(cosbb**2)*(ad1*(ad1-mu*tanbsb)/(sbot1**2-sbot2**2))**2
     &          * g(sbot12,sbot22) 

      vh3t(1,1) = i_mt4/(sinbt**2)
     &  * (mu*(-au1+mu/tanbst)/(stop1**2-stop2**2))**2*g(stop12,stop22)

      vh3b(1,1) = vh3b(1,1) + i_mz**2*(2*i_mb2*f1b-sqrt(i_mb2)*ad1*f2b)

      vh3t(1,1) = vh3t(1,1) + i_mz**2*(sqrt(i_mt2)*mu/tanbst*f2t)  

      vh3t(2,2) = i_mt4/(sinbt**2)
     &  * ( log(stop1**2*stop2**2/(mq2+i_mt2)/(mur2+i_mt2)) 
     &     + 2.D0 * (au1*(au1-mu/tanbst)/(stop1**2-stop2**2))
     &            * log(stop1**2/stop2**2))
     &     + i_mt4/(sinbt**2)*(au1*(au1-mu/tanbst)/(stop1**2-stop2**2))**2
     &          * g(stop12,stop22) 

      vh3b(2,2) = i_mb4/(cosbb**2)
     &  * (mu*(-ad1+mu*tanbsb)/(sbot1**2-sbot2**2))**2*g(sbot12,sbot22)

      vh3t(2,2) = vh3t(2,2) + i_mz**2*(-2*i_mt2*f1t+sqrt(i_mt2)*au1*f2t)

      vh3b(2,2) = vh3b(2,2) - i_mz**2*sqrt(i_mb2)*mu*tanbsb*f2b  
 
      vh3t(1,2) = -i_mt4/(sinbt**2)*mu
     &  *(au1-mu/tanbst)/(stop1**2-stop2**2)
     &  * ( log(stop1**2/stop2**2)
     &     + au1*(au1 - mu/tanbst)/(stop1**2-stop2**2)*g(stop12,stop22))

      vh3b(1,2) = -i_mb4/(cosbb**2)*mu
     &  *(au1-mu*tanbsb)/(sbot1**2-sbot2**2)
     &  * ( log(sbot1**2/sbot2**2)
     &     + ad1*(ad1 - mu*tanbsb)/(sbot1**2-sbot2**2)*g(sbot12,sbot22))

      vh3t(1,2) = vh3t(1,2)
     & +i_mz**2*( i_mt2/tanbst*f1t-sqrt(i_mt2)*(au1/tanbst+mu)/2.D0*f2t)

      vh3b(1,2) = vh3b(1,2)
     & +i_mz**2*(-i_mb2*tanbsb*f1b+sqrt(i_mb2)*(ad1*tanbsb+mu)/2.D0*f2b)
      ...
      do i = 1,2
         do j = 1,2
            vh(i,j) = 6.D0/(8.D0 * i_pi**2 * (h1t**2+h2t**2))
     &                    * vh3t(i,j) * 0.5D0 * (1.D0-al(i,j)*tt/2.D0) 
     &              + 6.D0/(8.D0 * i_pi**2 * (h1b**2+h2b**2))
...
c input : ma,tb,i_mw,i_mz
      subroutine BORNH(ma,tb,i_mw,i_mz,mhl,mhh,mhc,sa,ca)
     ...
     &      ,i_mw,i_mz,tb
      ...
      delta = sqrt( (ma**2+i_mz**2)**2 - (2.D0*ma*i_mz*cbb)**2 ) 
      mhl = sqrt( (ma**2 + i_mz**2 - delta)/2.D0 )
      mhh = sqrt( (ma**2 + i_mz**2 + delta)/2.D0 )
      mhc = sqrt( ma**2 + i_mw**2 )

      caa = -cbb * ( (ma**2-i_mz**2)/(mhh**2-mhl**2) )      
