...
ctp      COMMON/QMASSES/AMS,AMC,AMB,AMT
     ...
     .       - B1(NF)*G1(NF)/B0(NF)**2 - B2(NF)*G0(NF)/B0(NF)**2)/2D0
      TRAN(X,XK)=1D0+4D0/3D0*ALPHAS_OLD(X,2)/PI
     .          + XK*(ALPHAS_OLD(X,2)/PI)**2
...
123     AMS  = (AMSU+AMSD)/2
        AM(3) = AMS
        XMSB = AMS/CQ(ALPHAS_OLD(AMS,2)/PI,3)
     .            *CQ(ALPHAS_OLD(1.D0,2)/PI,3)/TRAN(AMS,0D0)
        DD = (XMSB-AMSB)/AMSB
...
ctp       XMHAT = 0
      ...
      YMSB(3) = AMSB
      IF(NF.EQ.3)THEN
       YMSB(4) = YMSB(3)*CQ(ALPHAS_OLD(AM(4),2)/PI,3)/
     .                   CQ(ALPHAS_OLD(1.D0,2)/PI,3)
       YMSB(5) = YMSB(4)*CQ(ALPHAS_OLD(AM(5),2)/PI,4)/
     .                   CQ(ALPHAS_OLD(AM(4),2)/PI,4)
       YMSB(6) = YMSB(5)*CQ(ALPHAS_OLD(AM(6),2)/PI,5)/
     .                   CQ(ALPHAS_OLD(AM(5),2)/PI,5)
      ELSEIF(NF.EQ.4)THEN
       YMSB(4) = XMSB
       YMSB(5) = YMSB(4)*CQ(ALPHAS_OLD(AM(5),2)/PI,4)/
     .                   CQ(ALPHAS_OLD(AM(4),2)/PI,4)
     ...
     .                   CQ(ALPHAS_OLD(AM(5),2)/PI,5)
      ELSEIF(NF.EQ.5)THEN
       YMSB(5) = XMSB
       YMSB(4) = YMSB(5)*CQ(ALPHAS_OLD(AM(4),2)/PI,4)/
     .                   CQ(ALPHAS_OLD(AM(5),2)/PI,4)
       YMSB(6) = YMSB(5)*CQ(ALPHAS_OLD(AM(6),2)/PI,5)/
     .                   CQ(ALPHAS_OLD(AM(5),2)/PI,5)
      ELSEIF(NF.EQ.6)THEN
       YMSB(6) = XMSB
       YMSB(5) = YMSB(6)*CQ(ALPHAS_OLD(AM(5),2)/PI,5)/
     .                   CQ(ALPHAS_OLD(AM(6),2)/PI,5)
       YMSB(4) = YMSB(5)*CQ(ALPHAS_OLD(AM(4),2)/PI,4)/
     .                   CQ(ALPHAS_OLD(AM(5),2)/PI,4)
      ENDIF
      IF(Q.LT.AMC)THEN
      ...
      ELSE
       XKFAC = 1D0
      ENDIF
      RUNM_ORIG = YMSB(N0)*CQ(ALPHAS_OLD(Q,2)/PI,N0)/
     .               CQ(ALPHAS_OLD(Q0,2)/PI,N0)
     .       * XKFAC
      RETURN
      END
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      DOUBLE PRECISION FUNCTION ALPHAS_OLD(Q,N)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION XLB(6)
      COMMON/ALSLAM_ORIG/XLB1(6),XLB2(6)
      COMMON/ALS_ORIG/XLAMBDA,AMC,AMB,AMT,N0
...
2      CONTINUE
      ...
      ENDIF
      IF(N.EQ.1)THEN
        ALPHAS_OLD=ALS1(NF,Q)
      ELSE
        ALPHAS_OLD=ALS2(NF,Q)
      ENDIF
      RETURN
      END
