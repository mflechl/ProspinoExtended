...
! ------------------------------
  function IFCT_TB_X12(dum) result(dsig)
    ...
    susy = 1.0d0                                                 ! switch for SUSY corrections
...
!tp       print*, " collider energy not large enough ",m1,m2,sqrt(sc)
    ...
    if (ii==3) then 
       del_s4 = eps_sli * m1**2                               &! unit is m^2
                   * (1.0d0-(m1+m2)**2/s)*(1.0d0-(m1-m2)**2/s)     ! rescale the s4 cutoff to be small everywhere
    else 
       del_s4 = 0.0d0
    end if
    ...
    case(3,4)                                                  ! real emission
       ...
       beta   = sqrt((1.0d0-del_s4/s)**2-(m1+m2)**2/s)          &
               *sqrt((1.0d0-del_s4/s)**2-(m1-m2)**2/s)
