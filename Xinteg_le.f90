...
! ------------------------------
  function IFCT_LE_X12(dum) result(dsig)
    ...
    real(kind=double)  :: sw,alpha_s,nlo,zero1,zero2,u1,u2,t1
    ...
    zero1  = 1.0d0                                               ! needed later to remove ps points in os subtraction 
    zero2  = 1.0d0
    ...
    theta_s3 = 0.0d0                                             ! the os subtraction theta function, only local
    if (s>4.0d0*m1**2) theta_s3 = 1.0d0                            ! intermediate stop

    gams   = ewi * m1                                          ! always a small width needed for arcustan 

    del_s4 = eps_sli * m1**2 * (1.0d0-m1**2/s)                   ! rescale the s4 cutoff 
    s4p    = 0.0d0                                               ! only for real corrections needed 
    s4     = 0.0d0                                               ! since it is used later on in massin
    s3     = 0.0d0                                               ! dto

    select case (ii)                                           ! all the phase spaces 
    case(-1,0,1,2)                                             ! born_lo, born_nlo, virt

       beta   = 1.0d0 - m1**2/s                                  ! t2 integration
       t2m    = -1.0d0/2.0d0 * ( s - m1**2 + s*beta )                
       t2p    = -1.0d0/2.0d0 * ( s - m1**2 - s*beta )
       t2     = var(3) * (t2p-t2m) + t2m
       t2_jac = t2p-t2m 

    case(3,4,7,8)                                              ! real(qg,gg,qq,qb), no os subtraction 
       
       beta   = (1.0d0-del_s4/s)**2 - m1**2/s                    ! t2 integration shifted 
       t2m    = -1.0d0/2.0d0 * ( s - del_s4 - m1**2 + s*beta )
       t2p    = -1.0d0/2.0d0 * ( s - del_s4 - m1**2 - s*beta )
       t2     = var(3) * (t2p-t2m) + t2m
       t2_jac = t2p-t2m 
    ...
    case(5,9)                                                  ! s3s on-shell subtraction
       ...
       s3p    = s + m1**2 - 2.0d0 * sqrt(s*m1**2)

       s3     = var(3) * (s3p-s3m) + s3m 
       s3_jac = s3p-s3m

       beta1   = (s-s3+m1**2)**2 - 4.0d0*m1**2*s 
       if ( beta1<0.0d0 ) then 
          n_faulty = n_faulty+1
          dsig = 0.0d0 
          return
       ...
       beta1   = sqrt( beta1 )
       s4m    = 1.0d0/2.0d0 * (s - s3 - m1**2 - beta1)
       s4p    = 1.0d0/2.0d0 * (s - s3 - m1**2 + beta1)
       s4     = var(4) * (s4p-s4m) + s4m
       s4_jac = s4p-s4m

       beta2   = abs( s-s4-m1**2 )
       t2m    = -1.0d0/2.0d0 * ( s - s4 - m1**2 + beta2 )          ! angular integration re-written
       t2p    = -1.0d0/2.0d0 * ( s - s4 - m1**2 - beta2 )
       t2     = var(5) * (t2p-t2m) + t2m 
       t2_jac = t2p-t2m 

       t2_jac = t2_jac * 2.0d0*(s4+m1**2)/s4/beta2               ! the additional jacobian
    ...
    case(6,10)                                                 ! s3s on-shell subtraction
       ...
       beta1s3   = sqrt( (s-s3s3+m1**2)**2 - 4.0d0*m1**2*s )
       s4s3m    = 1.0d0/2.0d0 * (s - s3s3 - m1**2 - beta1s3)
       s4s3p    = 1.0d0/2.0d0 * (s - s3s3 - m1**2 + beta1s3)
       s4s3     = var(4) * (s4s3p-s4s3m) + s4s3m
       s4s3_jac = s4s3p-s4s3m
          
       beta2s3  = abs( s-s4s3-m1**2 ) 
       t2s3m    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 + beta2s3 )    ! angular integration re-written
       t2s3p    = -1.0d0/2.0d0 * ( s - s4s3 - m1**2 - beta2s3 )
    ...
    if (iscaling==0)                                                                   &  ! scaling function
         dsig = dsig * 4.0d0/m1**2 * 4.0d0*pi*alpha_s
