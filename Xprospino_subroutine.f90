...
inv_scafac, scavar, scavar3, fix_mH, fix_mb, fix_tanbeta)
    ...
    double precision                     :: inv_scafac, fix_mH, ALPHAS_OLD, fix_mb, scavar, scavar3, fix_tanbeta
...
!----------------------------------------------------------------------------------------
    ...
    if (icoll == 0) then                                                                          ! set the collider energy Tevatron/LHC
       ...
       print*, " PROSPINO: icoll not set correctly?, continue LHC 14 TeV "
       sc = 14000.0**2 
    end if
...
!----------------------------------------------------------
    ...
    do ns   = ns_min,ns_max,10                                                                  ! change both scales simultaneously
       ...
       call INIT_SUSY(nin1,nin2,ipart1_in,ipart2_in,isq_ng_in,run0,run1,unimass,lowmass, fix_mH, fix_mb, fix_tanbeta)        ! get spectrum and initialize mass arrays.
       ...
       scafac = 1.0D0/inv_scafac ! scale to be used -> used for fac scale; qf=scafac * (m1+m2)
       scafac2 = 1.0D0/scavar    ! multiples of central scale -> used for ren scale; qr=(m1+m2) * scafac2 / 2.
       scafac3 = 1.0D0/scavar3   ! multiples of central scale -> used for b   scale; qb=(m1+m2) * scafac3 / 2.

       print*, "tan_b = ", tan_b, "   M_t = ", mt, "   M_b = ", mb
       print*, "M_H = ", mch
       !print*, "M_H = ",mch, "   tan_b = ", tan_b, "   M_t = ", mt, "   M_b = ", mb
       print*, "inverse factor for scale qf qr qb: ", 1.0D0/scafac, " ", 2.D0/scafac2, " ", 2.D0/scafac3
       print*,""
       ii = 0
       call INIT_ALPHAS
       call INIT_PDF(0)
       print*, "a_s(M_Z) = ", ALPHAS(mz,1), " at LO." 
       ii = 2
       call INIT_ALPHAS
       call INIT_PDF(0)
       print*, "a_s(M_Z) = ", ALPHAS(mz,2), " at NLO."
       print*,""
...
!       print*, ""
!       print*, "lo                   rel_error            nlo                   rel_error                 K               nlo(susy)"
!       print*, ""

!        print*, fct(0), " ", rel(0), " ", fct(20), " ", rel(20), " ", kfac, fct(22)
