...
!tp    end if
    ...
    if ( (icoll<0).or.(icoll>3) ) then                                                     ! test icoll 
       print*, " TEST_SETTINGS_GLOBAL: icoll not in range 0...3 ",icoll 
       lvalid_global = .false.
    end if
...
!                                                                        !
!-------------------------------------------------------------------------
  subroutine INIT_SUSY(nin1,nin2,ipart1_in,ipart2_in,isq_ng_in,run0,run1,unimass,lowmass, fix_mH, fix_mb, fix_tanbeta)
    
    integer,           intent(in) :: nin1,nin2
    integer,           intent(in) :: ipart1_in,ipart2_in,isq_ng_in
    real(kind=double), intent(in) :: run0,run1, fix_mH, fix_mb, fix_tanbeta
...
! note that in the old prospino there is routine INIT_ALPHAS_ARG(inlo) for an independent outside call
  subroutine INIT_ALPHAS
    ...
    integer           :: nq
    logical           :: inlo_alphas               ! note that inlo_alphas is an internal parameter
